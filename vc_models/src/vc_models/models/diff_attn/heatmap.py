...
from .utils import compute_token_merge_indices, cached_nlp, auto_autocast, compute_input_word_token_indices_from_sentence_tokens

__all__ = ['GlobalHeatMap', 'RawHeatMapCollection', 'SimpleHeatMapCollection', 'WordHeatMap', 'ParsedHeatMap', 'SyntacticHeatMapPair']
...
class GlobalHeatMap:
    ...
    def compute_word_heat_map_raw(self, word: str, word_idx: int = None, offset_idx: int = 0) -> WordHeatMap:
    ...
    def compute_word_heat_map(self, word: str, word_idx: int = None, offset_idx: int = 0) -> WordHeatMap:
        # merge_idxs, word_idx = compute_token_merge_indices(self.tokenizer, self.prompt, word, word_idx, offset_idx)
        merge_idxs, word_idx = compute_input_word_token_indices_from_sentence_tokens(self.tokenizer, self.prompt, word, word_idx, offset_idx)
...
class SimpleHeatMapCollection:
    def __init__(self, x_size, y_size):
        super().__init__()
        self._x_size = x_size
        self._y_size = y_size
        self._combined_heatmaps = None
        self.num_maps: int = 0
    
    def update(self, heatmap: torch.Tensor, num_maps: int = 1):
        with auto_autocast(dtype=torch.float32):
            heatmap = F.interpolate(heatmap, size=(self._x_size, self._y_size), mode='bicubic').clamp_(min=0)
            if self._combined_heatmaps is None:
                self._combined_heatmaps = heatmap
            else:
                self._combined_heatmaps += heatmap
        self.num_maps += num_maps

    def get_avg_heatmap(self):
        return self._combined_heatmaps / self.num_maps
